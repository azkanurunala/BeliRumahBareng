...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getCurrentDetail = (category: string): ProgressDetail => {
    // If we have progress detail in state, use it
    if (progressDetails[category]) {
      return progressDetails[category];
    }
    
    // For legal category, if progress detail doesn't exist, return default with 0% 
    // (will be auto-generated by API when loaded)
    if (category === 'legal') {
      return {
    ...
    // For funding category, if progress detail doesn't exist, return default with 0% 
    // (will be auto-generated by API when loaded)
    if (category === 'funding') {
      return {
  ...
  const ensureProgressDetail = useCallback(async (category: string): Promise<string | null> => {
    // Check if already exists (skip if it's a virtual ID)
    const existingId = progressDetailIds[category];
    if (existingId && !existingId.startsWith('virtual-')) {
      return existingId;
    }
